...
//  ResearchSwift
//
//  Created by iOS_Dev16 on 12/29/15.
//  Copyright © 2015 iOS_Dev16. All rights reserved.
//
import UIKit
...
class SmartImageView: UIImageView {
    ...
    var imageSource: SmartImageSource? {
        didSet {
            ...
            if let cached = SmartImageView.cachedImages[imageSource] where enabledCached {
                ...
                (imagePlaceHolder == nil) ? startGetImage(imageSource) : startGetImage(imageSource, imagePlaceHolder: imagePlaceHolder!)
            }
        }
    }
    ...
    private func startGetImage(imageSource: SmartImageSource, imagePlaceHolder: UIImage) {
        ...
        if enabledAsync {
            ...
            currentLoadOperation = imageSource.getImageAsync({[weak self] (image) -> Void in
                guard let weakSelf = self else { return }
                if weakSelf.enabledResizeImage {
                    weakSelf.image = image?.resizedImage()
                }
                else {
